...
def hypervolume(ref_point: np.ndarray, points: List[npt.ArrayLike]) -> float:
    ...
    if points_array.shape[1] != len(ref_point):
        raise ValueError(f"Dimension mismatch: ref_point has {len(ref_point)} dims, points have {points_array.shape[1]}")

    # Handle 1D case directly (pymoo segfaults on 1D)
    if len(ref_point) == 1:
        # For 1D maximization: HV = max(0, best_point - ref_point)
        best_point = np.max(points_array[:, 0])
        return max(0.0, best_point - ref_point[0])

    return HV(ref_point=ref_point * -1)(points_array * -1)
