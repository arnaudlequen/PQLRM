    ...
    def train(
        ...
        while self.global_step < total_timesteps:
            ...
            while not (terminated or truncated) and self.global_step < total_timesteps and local_step < max_local_steps:
                ...
                #TODO : comparer reward imm avec avg
        ...
        return self.get_local_pcs(state=eval_env.start_state_index)

    def _eval_all_policies(self, env: gym.Env) -> List[np.ndarray]:
        """Evaluate all learned policies by tracking them."""
        pf = []
        for vec in self.get_local_pcs(state=env.start_state_index):
        ...
        while not (terminated or truncated)\
            ...
            for action in range(self.num_actions):
                ...
                for q in non_dominated_set:
                    ...
                    #print(f"STATE : {state}, ACTION : {action}, REWARD : {im_rew}, NON-DOMINATED : {q}, DISTANCE : {dist}")
            ...
            #actions_followed[(state, closest_action)] = candidate_vector
            actions_followed.append((state, closest_action, candidate_vector, selected_im_rew, dist))
