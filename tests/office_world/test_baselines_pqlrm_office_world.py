...
from environments.office_world import OfficeWorld, time_penalty
...
def rm_get_coffee_no_hit_deco():
    ...
    rm.add_transition(0, 1, "coffee&!decoration", ConstantRewardFunction(1)) 
...
def rm_no_hit_deco():
    ...
    rm.add_transition(0, rm.terminal_u, "decoration", ConstantRewardFunction(-1)) 
    rm.add_transition(rm.terminal_u, rm.terminal_u, "True", ConstantRewardFunction(0))
    rm.finalize()
    rm.add_reward_shaping(0.9, 0.9)
    #rm.pretty_print()
...
def main():
    ...
    if log:
        ...
        env = OfficeWorld(map=env_map, reward_sources=[time_penalty, task_coffee, task_mail], render_mode='ansi')
