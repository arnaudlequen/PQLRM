...
def test_qrm_office_shared(map_name: str = "default_office") -> None:
    ...
    rms        = [rm_no_deco, rm_coffee, rm_mail]
    task_names = ["No-deco", "Coffee", "Mail"]
    ...
    # ------------------------------------------------------------------
    agents = [
        QRMAgent(rm_mail,        N_states, N_actions, alpha=0.1, gamma=0.95,  epsilon=0.1),
        QRMAgent(rm_coffee,      N_states, N_actions, alpha=0.1, gamma=0.95, epsilon=0.1),
        QRMAgent(rm_no_deco, N_states, N_actions, alpha=0.1, gamma=0.95, epsilon=0.1),
    ]

    # ------------------------------------------------------------------
    # 4. Trainer — override run_episode to use shared env correctly
    # ------------------------------------------------------------------
    trainer = SharedEnvTrainer(agents, make_shared_env, max_steps_per_episode=500)
