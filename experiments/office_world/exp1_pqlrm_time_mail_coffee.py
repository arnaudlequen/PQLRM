"""
Exp 1, PQLRM: time penalty + get_mail + get_coffee.

Trains for `--seed-nbr` seeds and writes one convergence trace per seed into
dataviz/data/exp1/pqlrm_time_mail_coffee.csv (hypervolume + cardinality).
"""
...
def main():
    ...
    parser.add_argument("--save-policies", action="store_true",
                        help="Also save tracked policies to JSON for the last seed.")
    ...
    for seed in range(args.seed_start, args.seed_start + args.seed_nbr):
        ...
        env = OfficeWorld(
            map=env_map,
            reward_sources=[time_penalty, task_mail, task_coffee],
            render_mode="ansi",
        )
